...
@section('page-title', 'Edit Paket Produk')
...
<div class="page-header-enhanced">
    ...
    <div class="page-header-main">
        <div style="display:flex;align-items:flex-start;gap:16px;">
            ...
            <div>
                <h1>Edit Paket: {{ $bundle->name }}</h1>
                <p class="subtitle">SKU: <code>{{ $bundle->sku }}</code></p>
            </div>
        </div>
    </div>
</div>

<form method="POST" action="{{ route('inventory.bundles.update', $bundle) }}">
@csrf @method('PUT')
        ...
        <div class="card">
            ...
            <div class="card-body" style="display:flex;flex-direction:column;gap:14px;">
                ...
                <div>
                    ...
                    <input type="text" name="name" class="form-control" required value="{{ old('name', $bundle->name) }}">
                </div>

                <div>
                    <label class="form-label">Deskripsi</label>
                    <textarea name="description" class="form-control" rows="2">{{ old('description', $bundle->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="category_id" class="form-select">
                        ...
                        <option value="{{ $cat->id }}" {{ old('category_id', $bundle->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="bundle_price" id="bundlePriceInput" class="form-control" required min="0"
                        value="{{ old('bundle_price', $bundle->bundle_price) }}" oninput="updateSummary()">
                </div>

                <div>
                    <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
                        <input type="checkbox" name="is_active" value="1" {{ $bundle->is_active ? 'checked' : '' }}>
                        Paket Aktif (muncul di kasir)
                    </label>
                </div>

                <button type="submit" class="btn btn-primary" style="width:100%;height:44px;">💾 Simpan Perubahan</button>
                <a href="{{ route('inventory.bundles.index') }}" class="btn btn-secondary" style="width:100%;text-align:center;">Batal</a>
            </div>
        </div>
    </div>
...
let bundleItems = @json($bundle->items->map(fn($i) => [
    'variantId' => $i->product_variant_id,
    'name' => $i->productVariant->product->name,
    'variant_label' => $i->productVariant->variant_label,
    'price' => (float)$i->productVariant->selling_price,
    'qty' => $i->quantity,
    'idx' => $i->id,
]));
let itemIndex = {{ $bundle->items->count() + 10 }};
...
function addItem(variantId) {
    if (bundleItems.find(i => i.variantId === variantId)) { alert('Varian sudah ada!'); document.getElementById('variantDropdown').style.display='none'; return; }
...
function renderTable() {
    ...
    if (!bundleItems.length) {
        tbody.innerHTML = `<tr><td colspan="5" class="text-center py-3 text-muted" style="font-style:italic;">Belum ada item.</td></tr>`;
    ...
    tbody.innerHTML = bundleItems.map(item => `
        <tr>
            ...
            <td><button type="button" onclick="removeItem(${item.variantId})" style="background:none;border:none;color:var(--color-danger);cursor:pointer;font-size:18px;">✕</button></td>
        </tr>
    `).join('');
    updateSummary();
}
...
function updateSummary() {
    ...
    if (discount > 0) { badge.style.display='block'; document.getElementById('discountBadgeText').textContent=`Hemat ${discount}%`; }
